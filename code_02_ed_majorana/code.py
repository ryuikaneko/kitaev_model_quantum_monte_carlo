...
def make_hamiltonian(Lx, Ly, bonds):
    """
    Make the full Kitaev Hamiltonian on the Type-II boundary chain of 2*L sites.
...
def main():
    ...
    etas = make_etas(nbondz)
    enes = []
    for i in range(2**nbondz):
        H = make_one_body_hamiltonian(args.L, args.L, bonds, etas[i])
        eigvals = np.linalg.eigvalsh(H)
        epsilon = eigvals[:eigvals.shape[0]//2]
        ene = 0.5 * etas @ epsilon # sum_{i=1}^{N/2} epsilon_i (n_i - 1/2)
        enes.extend(ene)
    enes = np.sort(np.array(enes))
    eigvals = enes
    #
#    H = make_hamiltonian(args.L,args.L,bonds)
#    eigvals = np.linalg.eigvalsh(H)
#    print(eigvals)
#    print(np.allclose(enes,eigvals))
